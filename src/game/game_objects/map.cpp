...
game::gameobjects::Map::Map(WINDOW* parent_window, uint height, uint width) : GameObject(0, 0, height, width) {
...
bool game::gameobjects::Map::is_city_at(uint pos_height, uint pos_width) {
    for (auto & city: cities) {
...
game::gameobjects::City* game::gameobjects::Map::get_city_at(uint pos_height, uint pos_width) {
    for (auto & city: cities) {
        if (city.is_point_within_city(pos_height, pos_width)) {
            return &city;
        }
